...
public class BoardDAO 
{
	...
	private final String BOARD_INSERT = 
		"insert into board (id, title, write, cont) "
	...
	private final String BOARD_UPDATE = "update board set title= ?, write= ? , cont = ? where id = ?"; 
	...
	/*
	 * // 글 조회수 늘리는 메소드 public void addCNT(BoardDTO dto) {
	 * 
	 * try { conn = JDBCUtil.getConnection(); // ADD_CNT
	 * ="update board set cnt = cnt + 1 where seq = ?" pstmt =
	 * conn.prepareStatement(ADD_CNT);
	 * 
	 * pstmt.setInt(1, dto.getId());
	 * 
	 ...
	 * }
	 * 
	 * catch (Exception e) { System.out.println("조회수 입력 실패"); e.printStackTrace();
	 * 
	 * }
	 * 
