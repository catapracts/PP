...
public class Board2DAO 
{
	// DAO : DataBase의Board 테이블을 접근 하는 객체 : SQL 쿼리 
			...
			private final String BOARD_INSERT = 
				"insert into board2 (id, title, write, cont) "
			...
			private final String BOARD_UPDATE = "update board2 set title= ?, write= ? , cont = ? where id = ?"; 
			
			// DB의 Board 테이블의 레코드를 삭제 
			private final String BOARD_DELETE = "delete board2 where id = ?";
			
			/*
			 * //글 조회수 늘리는 쿼리 private final String ADD_CNT
			 * ="update board2 set cnt = cnt + 1 where id = ?";
			 */
			
			
			//insertBoard(Board2DTO dto) 메소드  : 
			public void insertBoard (Board2DTO dto) 
			{
			...
			// DB의 레코드 하나를 Board2DTO에 담는다. 각각의 Board2DTO 를 ArrayList에 담아서 리턴 
			// rs, pstmt, conn
			public List<Board2DTO> getBoardList(Board2DTO dto) 
			{
				//중요 : ArryList 는 While 블락 밖에서 선언
				//      ArryList에 저장되는  Board2DTO 선언은 while 블락 내부에서 선언 
				...
				try 
				{
					...
					// rs의 각 레코드를 Board2DTO에 저장 ==> 각 각의 DTO를 ArrayList에 저장 
						// if , do ~ while   <===>  while
					...
					while (rs.next()) 
					{
						...
						// boardList : ArrayList의 add 메소드를 사용해서 Board2DTO를 저장함. 
						boardList.add(board); 	
					}
			...
			public Board2DTO getBoard(Board2DTO dto) 
			{
			...
			public void updateBoard(Board2DTO dto) 
			{
			...
			public void deleteBoard (Board2DTO dto) 
			{
