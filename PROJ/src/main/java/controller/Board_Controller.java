...
// http://localhost:8081/PROJ/*.board
@WebServlet ("*.board")
public class Board_Controller extends HttpServlet 
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		System.out.println(" board 요청을 처리하는 controller 입니다. ");
		
		//	URL : http://localhost:8181/JSP_MVC_M2/my.board
		// 	URI : /JSP_MVC_M2/my.board
		...
		if (path.equals("/insertBoard.board")) 
		{
			System.out.println("/insertBoard.board 요청");
			//로직 처리 
			...
			*/ 
			...
			// 4. 뷰 페이지 전송 : 값을 insertBoard 후 DB의 전체 레코드를 출력 페이지로 이동 
				// 클라이언트가 getBoardList.board 요청을 새롭게 요청함. 
				//주의 : http://localhost:8181/getBoardList.board   <== 오류 
				    //  http://localhost:8181/JSP_MVC_M2/getBoardList.board   <== 정상  
			response.sendRedirect("getBoardList.board"); 
			
					
		}
		
		else if (path.equals("/getBoardList.board")) 
		{		// DB의 레코드를 출력 하는 페이지 
			System.out.println("/getBoardList.board 요청");
			//로직 처리 
		...
		else if (path.equals("/getBoard.board")) 
		{
			System.out.println("/getBoard.board 요청");
			//로직 처리 
			
			//1. clinet 넘어오는 파라미터 seq 변수의 값을 읽어서 dto에 저장후 dao.getBoard(dto) 
			//http://localhost:8181/JSP_MVC_M2/getBoard.board?seq=5 
		...
		else if (path.equals("/updateBoard.board")) 
		{
			System.out.println("/updateBoard.board 요청");
			//로직 처리 
		...
		else if (path.equals("/deleteBoard.board")) 
		{
			System.out.println("/deleteBoard.board 요청");
			//로직 처리 
